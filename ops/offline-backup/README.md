...
| `drive.ts:163`                                  | `chown -R $USER:$USER` on the mount point, derived from the environment                                                                                                                                   | no `chown` at all unless the caller names an owner                                                                   |
...
| `sync.ts:93`                                    | `line.match(/([\d.]+[KMGT]?)/)` against a progress line matches the _percentage_ first, so the "size" printed was the percentage                                                                          | `parseRsyncProgress` reads the size and speed columns, with a test pinning that the percentage is not read as a size |
