...
/** Default number of trailing code points {@link maskKey} leaves visible. */
...
export class CryptoService implements SecretCipher {
  ...
  static fromHexKey(hex: string): CryptoService {
    if (typeof hex !== "string") {
      throw new CryptoError(CryptoErrorCode.InvalidHexKey, "encryption key must be a hex string")
    }
    const normalised = hex.toLowerCase()
    if (normalised.length !== HEX_KEY_LENGTH || !HEX_PATTERN.test(normalised)) {
      throw new CryptoError(
        CryptoErrorCode.InvalidHexKey,
        `encryption key must be exactly ${HEX_KEY_LENGTH} hex characters (${AES_KEY_BYTES} bytes)`,
      )
    }
    // The pattern check above makes a decode failure impossible, so the decoder
    // needs no error path of its own. The `#keyBytes` assignment replaces the
    // constructor's SHA-256 source before the instance is handed out.
    const bytes = decodeHex(normalised)
  ...
  async decrypt(ciphertext: string): Promise<string> {
    ...
    try {
      ...
      const opened = await crypto.subtle.decrypt(
        { name: AES_ALGORITHM, iv: sealed.subarray(0, IV_LENGTH) },
        key,
...
/**
 * Display hint for a key: the last `visible` code points, everything before them
 * replaced by asterisks. Returns the same number of code points as `key` and
 * never the whole key — when `key.length <= visible` (or `visible` is not a
 * positive integer, e.g. `0` or `-1`) the result is all asterisks. Masking is
 * per code point (`Array.from`), so a surrogate pair or emoji is never split.
 * An empty key masks to the empty string. A non-string argument is refused with
 * {@link CryptoErrorCode.InvalidSecret} rather than coerced.
 */
export function maskKey(key: string, visible: number = DEFAULT_MASK_VISIBLE): string {
  ...
  const shown = Number.isInteger(visible) && visible > 0 ? visible : 0
  if (shown === 0 || shown >= points.length) return "*".repeat(points.length)
  return "*".repeat(points.length - shown) + points.slice(-shown).join("")
}
