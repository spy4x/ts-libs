...
| the fix above treated every thrown error as a dead connection, so one ordinary error reply from Redis (`WRONGTYPE`, an out-of-memory refusal, `READONLY`, `BUSY`) disabled an otherwise healthy store for good | `keeps working after Redis answers one command with an error`                                                                                                                                                                 |
