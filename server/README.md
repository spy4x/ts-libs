...
| every method called the client directly after `close()` or after the connection died, crashing the process with an uncaught `BadResource` instead of rejecting                           | `rejects instead of crashing after close()` and `rejects instead of crashing once the connection is killed` |
...
string reached `.slice(64)` on that value and threw `TypeError: name.slice is not a function` — from
...
the repository only ever reads the table, never creates it.
